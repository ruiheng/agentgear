...
11. **Resolved: avoid refetching unchanged upstream skills on every source install.** A default `agentgear-source-install` reuses the upstream skill from `current` or another recorded release only when that runtime's catalog has the exact same repository, ref, commit, skill path, and catalog-anchored content digest. Modified cached content is rejected. First installation and changed pins use a fresh filtered, shallow fetch and materialize only the declared skill path.
