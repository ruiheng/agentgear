...
The default targets are `general,gemini,agy,claude`. `general` installs the common
...
`~/.gemini/skills` for Gemini CLI; its project destination remains
`.gemini/skills`. `agy` installs globally to the directly discovered
`~/.gemini/config/skills` directory. In projects it shares the common
`.agents/skills` directory with `general`, so equivalent target roots are
installed only once. Normal installation does not maintain
...
The installer places skills only. It does not silently write host hooks, MCP configuration, or
permission rules. Configure permissions explicitly through
