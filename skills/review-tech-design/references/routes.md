...
and each reviewed target by Task and Round. Round 1 carries the full
...
one request per Task/Round. Continuing to the next round is workflow state, not a
User Decision Delta.

Apply `review-contract` before opening either target.
