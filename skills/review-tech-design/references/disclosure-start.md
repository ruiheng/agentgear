...
- For `design_spec_review_requested` with `Mode: draft-round`, retrieve `agentgear skill get review-tech-design draft-round-review`, then `message-delivery` when reporting or recovering context.
- For `design_spec_review_requested` with `Mode: committed-docs`, retrieve `agentgear skill get review-tech-design committed-docs-review`, then `message-delivery` when reporting.
- Every other invocation is direct-use mode; review the readable target named by the user and retrieve `message-delivery` for the report form. Do not send Waypost.
