...
- add `--merge-mode ff|no-ff` when a merge commit is wanted instead of the default `ff-only` fast-forward: `ff` creates one only when fast-forward is impossible, `no-ff` always
