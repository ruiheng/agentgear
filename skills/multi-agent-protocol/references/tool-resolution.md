...
Use this reference only before creating a target collaborator session. With the current Agent Deck adapter, creation uses `agent_deck_create_session`.
...
- `<target_workdir>` is the same workdir passed to the session manager's create operation.
