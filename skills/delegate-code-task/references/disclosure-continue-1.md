...
- When execution depends on local setup, inspect the repository's documented configuration, `.env` files, package scripts, runtime metadata, and available tool installations before asking for help. A temporary worktree lacks non-versioned files such as `.env`; copy required ones from the recorded `Planner workspace` when a check needs them. Do not expose secret values in messages.
...
- Follow workflow policy. After a delivery commit, retrieve `agentgear skill get review-request` and use its standard send flow when per-task review is required; reuse the recorded reviewer. The required handoff is a real `review_requested` delivery whose body contains the declared `Action: review_requested` envelope. Do not substitute an ad-hoc `waypost_send` message such as `Review lane: ...` / `Commit: ...`, or call a progress note a review request. The coder request does not need task background, goals, constraints, workflow policy, or other task-content description; reviewer gets them from planner context.
- On a matching `Action: rework_required` from the recorded reviewer, continue this same task on the recorded branch, implement the actionable findings, validate, and submit the next canonical review request to the same reviewer. The dispatch contract already authorizes this continuation; do not ask the planner whether to continue, replace the review request with a progress note, or stop merely because the previous round was rejected. Keep the delivery claimed until the next review request (or a documented blocker handoff) succeeds.
