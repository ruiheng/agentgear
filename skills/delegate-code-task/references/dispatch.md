...
For `temporary; cleanup=planner`, require `task_dir` and `worker_workspace` to resolve to the same path before dispatch. For any lane-exclusive workspace (`temporary` or `dedicated`), create it before workspace preparation (e.g., `git worktree add --detach <task_dir>`).
