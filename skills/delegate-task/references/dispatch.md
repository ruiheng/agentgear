...
3. For a Waypost worker:
   ...
   - record the returned real id and sole address, then call `waypost_send` with `from_address = waypost_status.default_sender` and `to = <that address>`, subject `delegate: <task_id> -> worker`
