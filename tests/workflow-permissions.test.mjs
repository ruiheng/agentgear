...
test("workflow permissions use the stable launcher and never an old source path", () => {
  ...
  try {
    ...
    assert.equal(claude.permissions.allow.includes("Bash(agentgear run remember *)"), true);
    assert.equal(claude.permissions.allow.includes("Bash(~/.local/bin/agentgear run remember *)"), true);
    ...
    assert.match(generated[1], /pattern = \["agentgear", "run", "remember"\]/);
    ...
    assert.match(generated[2], /commandPrefix = \["agentgear", "run", "remember"\]/);
...
test("permission migration detects every prior launcher form missing skill get and retires review-tech-design grants", () => {
  ...
  try {
    ...
    assert.equal(stale.issues.every(issue => /9 workflow launcher approval/.test(issue)), true);
