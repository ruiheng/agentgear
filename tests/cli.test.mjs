...
test("installs, launches, and safely uninstalls a managed core skill", () => {
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "ai-skills-cli-test-"));
  ...
  try {
    ...
    const launcher = path.join(home, ".local", "bin", "ai-skills");
...
test("workflow installation provisions its explicit helper commands", () => {
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "ai-skills-workflow-test-"));
  const home = path.join(temporary, "home");
  const environment = {
    HOME: home,
    XDG_DATA_HOME: path.join(temporary, "data"),
  ...
  try {
    ...
    for (const helper of ["agent-deck-workflow-init-permissions", "adwf-send-and-wake"]) {
      ...
      assert.match(fs.readlinkSync(helperPath), /ai-skills[\\/]current[\\/]skills[\\/]agent-deck-workflow/);
    }
  } finally {
    fs.rmSync(temporary, { recursive: true, force: true });
  }
...
test("purge removes managed skills and runtime artifacts but preserves unowned files", () => {
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "ai-skills-purge-test-"));
  const home = path.join(temporary, "home");
  const environment = {
    HOME: home,
    XDG_DATA_HOME: path.join(temporary, "data"),
  ...
  const dataRoot = path.join(environment.XDG_DATA_HOME, "ai-skills");
  const localBin = path.join(home, ".local", "bin");
  const unmanagedSkill = path.join(home, ".agents", "skills", "not-managed-by-ai-skills");
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(localBin, "ai-skills")), true);
    assert.equal(fs.existsSync(path.join(dataRoot, "current")), true);

    const purge = childProcess.spawnSync(process.execPath, [path.join(localBin, "ai-skills"), "uninstall", "--purge"], {
      cwd: rootDir,
      encoding: "utf8",
    ...
    assert.equal(pathExists(path.join(localBin, "ai-skills")), false);
    ...
    assert.equal(fs.existsSync(path.join(environment.XDG_STATE_HOME, "ai-skills", "installs.json")), false);
  } finally {
    fs.rmSync(temporary, { recursive: true, force: true });
  }
});
...
test("purge refuses locally changed skills until forced", () => {
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "ai-skills-purge-safety-test-"));
  const home = path.join(temporary, "home");
  const environment = {
    HOME: home,
    XDG_DATA_HOME: path.join(temporary, "data"),
...
test("targeted purge retains shared runtime while another target remains managed", () => {
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "ai-skills-targeted-purge-test-"));
  const home = path.join(temporary, "home");
  const environment = {
    HOME: home,
    XDG_DATA_HOME: path.join(temporary, "data"),
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(environment.XDG_STATE_HOME, "ai-skills", "installs.json")), true);
...
test("developer links stay live while release installs become independent copies", () => {
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "ai-skills-channel-test-"));
  const home = path.join(temporary, "home");
  const environment = {
    HOME: home,
    XDG_DATA_HOME: path.join(temporary, "data"),
...
test("a developer checkout applies edits live, then update freezes that exact revision", async () => {
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "ai-skills-live-update-test-"));
  ...
  try {
    ...
    const { main: checkoutMain } = await import(`${pathToFileURL(path.join(checkout, "cli", "ai-skills.mjs")).href}?test=${Date.now()}`);
